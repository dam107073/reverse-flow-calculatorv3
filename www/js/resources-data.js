  ...
  function normalizeCoursePayload(body) {
    ...
    const items = body.course.lessons.map((item, lessonIndex) => {
      ...
      lesson.steps = item.steps.map((step, stepIndex) => {
        ...
        if (normalized.type === "teaching") {
          Object.assign(normalized, { title: text(step.title), statement: text(step.statement), body: text(step.body), kind: text(step.kind) });
          if (step.interaction && text(step.interaction.type) === "guided-practice") Object.assign(normalized, { kind: "guided-practice", prompt: text(step.interaction.prompt), choices: array(step.interaction.choices).map(text), correctIndex: Number(step.interaction.correctIndex), feedback: text(step.interaction.feedback), concept: text(step.interaction.concept) });
        }
        ...
        if (normalized.type === "teaching") {
          required(normalized.title && normalized.statement && (!normalized.kind || ["worked-example", "guided-practice"].includes(normalized.kind)), `Teaching step ${normalized.id} is incomplete.`);
          if (normalized.kind === "guided-practice") required(normalized.prompt && normalized.feedback && normalized.concept && normalized.choices.length === 4 && new Set(normalized.choices).size === 4 && Number.isInteger(normalized.correctIndex) && normalized.correctIndex >= 0 && normalized.correctIndex < 4, `Guided practice step ${normalized.id} has invalid answers.`);
        }
